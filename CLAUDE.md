...
- Everything currently lives in `src/App.tsx` — split into components as it grows

## Commands
- `npm install`
- `npm run dev` — dev server, port 5173
- `npm run typecheck` — type-check only, no emit
- `npm run build` — type-checks, then production build

## Conventions
- Color tokens live in the `C` object at the top of App.tsx — reuse these, don't
...
`computePosition(supplies, borrows)` in App.tsx is the real math: collateral value,
...
It lists reserves matching the `ASSETS` table in App.tsx exactly — USDG is the only
...
- Replace hardcoded `ASSETS` array with a live price feed (Chainlink/Pyth), matching
  the mock price feeds already used in `contracts/`
- Split App.tsx into per-component files
